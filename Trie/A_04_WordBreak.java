...
public Boolean isWordPresent(String s, TrieNode cur, int l, int n, String curWord) {
  if (l >= n) {
    System.out.println("i am here: " + curWord.length() + ", curWord: " + curWord + ", l: " + l);
    return curWord.length() == 0;
  }
  ...
  if (cur.isEndOfWord) {
    if (isWordPresent(s, root, l + 1, n, "")) {
      return true;
    }
  }

  return isWordPresent(s, cur, l + 1, n, curWord);
}
...
public static boolean search(TrieNode root, String word) {
  ...
  for (int i = 0; i < word.length(); i++) {
    int index = word.charAt(i) - 'a';
    // if any charector of string does not exists in tree, we can confirm that word
    // does not exists
    if (cur.childNodes[index] == null) {
      return false;
    }
  ...
  // if all chars exists, one that confirms the existance of word is flag
  // 'endOfWord'
  return cur.isEndOfWord;
}
