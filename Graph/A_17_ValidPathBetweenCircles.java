...
public class A_17_ValidPathBetweenCircles {
   ...
   */
    ...
    //loop though each cell against each circle to mark all the cells which are inside the circe, 
    // by using formula [ (cell.x-circlex)^2 + (cell.y-circley)^2 <= r*r ], that means point is inside the circle
    for(int i=0; i<=n; i++){
      for(int j=0; j<=m; j++){
		...
		//check for initial points exists inside circle
