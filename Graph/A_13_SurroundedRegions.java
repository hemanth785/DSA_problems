...
public class A_13_SurroundedRegions {
  public void solve(char[][] board) {
    int n = board.length;
    int m = board[0].length;

    int notPossible[][] = new int[n][m];
    // traverse boundary of board and mark all the cells with "O" which are connected to the boundary cells 'O' as not possible
    // Then all the remaining cells with 'O', inside boundary we can convert it into 'X'
    // check top row
    int row = 0;
    for (int i = 0; i < m; i++) {
