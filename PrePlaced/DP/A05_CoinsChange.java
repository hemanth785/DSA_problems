...
public class A05_CoinsChange {
  public int coinChange(int[] coins, int amount) {
    ...
    for (int[] row : dp) {
      // here using -2 as default, because -1 is already used for indicationg invalue coin combination
      Arrays.fill(row, -2);
    }
  ...
  public int coinsChangeMemo(int[] coins, int amount, int n, int[][] dp) {
    ...
    // if we are returning the -1, then we should not consider it as valid result. we should ignore it
    if (amount < 0 || n < 0) {
      return -1;
    }
